...
export default function JoinViaInvite({ roomCode, onJoin, onCancel }) {
  const [username, setUsername] = useState('');
  const [error, setError] = useState('');
  const [activeModal, setActiveModal] = useState(null); // 'how_to_play' | 'game_rules' | 'privacy_policy' | null
  ...
  return (
    <div className="relative min-h-screen bg-slate-950 flex justify-center items-center p-6 text-white select-none overflow-hidden">
      ...
      {/* 4. MAIN CARD */}
      <div className="max-w-md w-full bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl rounded-3xl p-8 shadow-[0_20px_50px_rgba(0,0,0,0.8)] flex flex-col items-center relative z-10 ring-1 ring-white/5">
        {/* Glow behind title */}
        <div className="absolute w-48 h-16 bg-red-600/10 rounded-full blur-2xl -top-4 pointer-events-none"></div>

        {/* Title */}
        <div className="flex flex-col items-center mb-6 relative select-none">
          <span className="font-extrabold text-6xl sm:text-7xl tracking-tighter italic bg-gradient-to-b from-amber-300 via-amber-400 to-orange-500 bg-clip-text text-transparent drop-shadow-[0_4px_12px_rgba(245,158,11,0.5)] transform -skew-x-6">
            UNO
          </span>
          <div className="relative mt-1 px-8 py-1.5 bg-gradient-to-r from-red-700 via-red-600 to-red-700 text-white font-black text-[10px] sm:text-xs tracking-[0.25em] uppercase transform -skew-x-12 border-y border-red-400/30 shadow-lg shadow-red-600/20">
            NO MERCY
          </div>
        </div>

        <h3 className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em] mb-1 mt-4">INVITATION TO JOIN LOBBY</h3>
        <span className="text-3xl font-black text-yellow-400 tracking-widest mb-6">{roomCode}</span>

        <form onSubmit={handleSubmit} className="w-full space-y-6">
          <div className="flex flex-col text-left">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Your Username</label>
            <div className="relative">
              <input
                ...
                placeholder="CARDKING_X"
                maxLength={16}
                className={`w-full bg-slate-950/80 border-2 rounded-xl pl-4 pr-10 py-3.5 text-slate-200 font-extrabold placeholder:text-slate-800 focus:outline-none transition-colors tracking-wide ${
                  error ? 'border-red-600 focus:border-red-500' : 'border-slate-800/80 focus:border-amber-500/80'
                }`}
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-700 pointer-events-none">
            ...
            {error && (
              <span className="text-xs font-bold text-red-500 mt-2 block animate-pulse">
                ⚠️ {error}
              </span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-3 pt-2">
            ...
            <button
              type="button"
              onClick={onCancel}
              className="w-full py-4 rounded-2xl bg-slate-950/60 hover:bg-slate-900 border border-slate-850 hover:border-slate-700 font-bold text-slate-450 hover:text-white hover:scale-[1.02] hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-300 shadow-md"
            >
              Cancel
            </button>
          </div>
        ...
        <div className="w-full flex justify-center items-center gap-4 pt-4 mt-6 border-t border-slate-800/50 text-[10px] font-black uppercase tracking-wider select-none text-slate-500">
          <button
            type="button"
            onClick={() => setActiveModal('how_to_play')}
            className="hover:text-amber-400 hover:drop-shadow-[0_0_6px_rgba(245,158,11,0.6)] transition-all duration-200 cursor-pointer focus:outline-none"
          >
            How to Play
          </button>
          <span>•</span>
          <button
            type="button"
            onClick={() => setActiveModal('game_rules')}
            className="hover:text-red-500 hover:drop-shadow-[0_0_6px_rgba(239,68,68,0.6)] transition-all duration-200 cursor-pointer focus:outline-none"
          >
            Game Rules
          </button>
          <span>•</span>
          <button
            ...
            className="hover:text-blue-400 hover:drop-shadow-[0_0_6px_rgba(96,165,250,0.6)] transition-all duration-200 cursor-pointer focus:outline-none"
          >
            Privacy Policy
          </button>
        </div>
