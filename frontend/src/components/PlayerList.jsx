...
export default function PlayerList({ players, hostId, currentUserId, onRemovePlayer }) {
  const isCurrentUserHost = currentUserId && hostId && currentUserId.toString() === hostId.toString();
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <div>
                {isHost ? (
                  <span className="bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 text-xs font-black px-3 py-1 rounded-full whitespace-nowrap">
                    ✓ Ready (Host)
                  </span>
                ) : player.isReady ? (
                  <span className="bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 text-xs font-black px-3 py-1 rounded-full whitespace-nowrap">
                  ...
                  <span className="bg-red-500/10 text-red-500 border border-red-500/20 text-xs font-black px-3 py-1 rounded-full whitespace-nowrap">
                    ✕ Not Ready
                  </span>
                )}
              </div>

              {isCurrentUserHost && !isHost && onRemovePlayer && (
                <button
                  onClick={() => onRemovePlayer(player)}
                  className="px-2.5 py-1 rounded-lg bg-red-650/20 hover:bg-red-600 text-red-400 hover:text-white border border-red-600/30 text-[10px] font-black uppercase transition-all duration-200 flex-shrink-0"
